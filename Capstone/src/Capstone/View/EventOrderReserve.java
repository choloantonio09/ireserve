...
public class EventOrderReserve extends JDialog implements  ListSelectionListener{
	...
	private JSpinner[] spnFood;
	/**
	 * Create the panel.
	 */
	public EventOrderReserve() {
		...
		btnSettle.setIcon(new ImageIcon(EventOrderReserve.class.getResource("/Images/Icon/plus.png")));
		...
		btnCancel.setIcon(new ImageIcon(EventOrderReserve.class.getResource("/Images/Icon/cancel.png")));
		...
		btnClose.setIcon(new ImageIcon(EventOrderReserve.class.getResource("/Images/Icon/exit.png")));
		...
		listPackage.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent arg0) {
				Thread thr=new Thread(new Runnable(){
					  ...
					  {
					      try{
					    	  if (listPackage.getFirstVisibleIndex() == listPackage.getSelectedIndex()) {
		...
		pnlItems.setLayout(new MigLayout("", "[grow][grow]", "[][250px][grow]"));
		...
		if(lblTakeDinein.getText().equalsIgnoreCase("Take-out")){

			CountTakeout cc = new CountTakeout();
			cc.getAccountNumber();
			if(!cc.getAccountNumber().isEmpty()){
				tfId.setText(cc.getAccountNumber());
			}	
		}else if(lblTakeDinein.getText().equalsIgnoreCase("Dine-in")){
	...
	public ModelSalesEventOrder getFieldData(){
...
//			lblFoodImage = new BackgroundImageApp[count+1];
			lblFood = new JLabel[count+1];
			spnFood = new JSpinner[count+1];
			...
			if(listSelect.equalsIgnoreCase("Categories")){
				...
				c.pst = c.con.prepareCall("{call select_event_package_food_via_category(?)}");						
			}
			...
			while(c.rs.next()){
				...
				pnlDynamic[iFood].setPreferredSize(new Dimension(100, 150));
				pnlDynamic[iFood].setLayout(new MigLayout("", "[]", "[]"));

				pnlDynamic[iFood].add(lblFoodImage, "cell 0 0 3 1,alignx center");
				
				lblFood[iFood] = new JLabel("[" + foodPackageType + "]");
				lblFood[iFood].setHorizontalAlignment(SwingConstants.LEADING);
				pnlDynamic[iFood].add(lblFood[iFood], "cell 0 1 3 1, ,alignx center");
				...
				btnFoods[iFood].setFont(new Font("Sylfaen", Font.PLAIN, 15));
				...
				pnlDynamic[iFood].add(btnFoods[iFood], "cell 0 2 3 1, grow");
	 ...
	 public void ViewPackageData(String pid, String pname, String total){
		 pnlFoodSizes.removeAll();
			...
			try {
				...
				c.pst = c.con.prepareCall("call select_all_normal_event_food_package_order(?)");
				c.pst.setString(1, pid);
				c.pst.execute();
				c.rs = c.pst.getResultSet();
				tblPackage.setModel(DbUtils.resultSetToTableModel(c.rs));
				tblPackage.removeColumn(tblPackage.getColumnModel().getColumn(5));
				c.rs.close();
				c.pst.close();
				c.con.close();
			} catch (SQLException e) {
			...
			pnlFoodSizes.add(spinner, "cell 0 0,grow");
			...
			tfTotalPackage.setFont(new Font("Sylfaen", Font.PLAIN, 30));
			tfTotalPackage.setColumns(10);
			pnlFoodSizes.add(tfTotalPackage, "cell 0 0,grow");
	 ...
	 public void ViewFoodSizes(String foodId){
		 ...
		 row = 0;
			...
			try {
				...
				c.rs = c.pst.getResultSet();
				pnlFoodSizes.removeAll();
				pnlFoodSizes.setLayout(new MigLayout("", "[]", "[]"));
				while(c.rs.next()){
					sizeName = c.rs.getString("normal_food_size");
					sizePackagePrice = c.rs.getString("normal_food_price");
					btnFoodSizes[row] = new JButton(sizeName + " " + sizePackagePrice);
					btnFoodSizes[row].setName(sizePackagePrice);
					btnFoodSizes[row].setFont(new Font("Sylfaen", Font.PLAIN, 15));
					...
					pnlFoodSizes.add(btnFoodSizes[row], "cell 0 "+cellY+",grow");
					...
					spnFoodSizes[row].setFont(new Font("Sylfaen", Font.PLAIN, 15));
